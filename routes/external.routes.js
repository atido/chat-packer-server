...
router.get('/accommodations', async (req, res, next) => {
  ...
  const result = await accommodationServiceInstance.searchAccommodations(location, checkin, checkout, adultsNb);
  res.status(200).json(result);
});

router.get('/flights', async (req, res, next) => {
  const { adultsNb, originIATA, destinationIATA, departureDate, returnDate } = req.query;
  const result = await flightServiceInstance.searchFlights(adultsNb, originIATA, destinationIATA, departureDate, returnDate);
  res.status(200).json(result);
});
